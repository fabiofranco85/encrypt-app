# Shipping Hushbox: Simulator → TestFlight → App Store

A practical, end-to-end guide to run Hushbox locally, test it on your own
iPhone via TestFlight, and publish it to the App Store.

> Hushbox is generated from `project.yml` with **XcodeGen** and uses
...
```bash
xcodegen generate      # creates Hushbox.xcodeproj from project.yml
...
1. In Xcode's toolbar, set the **scheme** to `Hushbox` (it should be selected
   automatically).
...
- **Decrypt**: paste the armored text back, or open the saved `.hushbox` file →
...
Great for checking haptics, the share sheet, "Open in Hushbox", and real
performance of the (intentionally slow) Argon2id KDF.

### 2a. Set your signing team

1. Open `Hushbox.xcodeproj`, select the **Hushbox** target ▸ **Signing &
   Capabilities**.
2. Check **Automatically manage signing**.
3. Choose your **Team** (your Apple ID's personal team works for development).
4. If the bundle id `com.hushbox.app` is taken (it must be globally unique for
   distribution), change it — e.g. `com.yourname.hushbox` — in
...
2. Fill in:
   - **Platform:** iOS
   - **Name:** Hushbox (must be unique across the App Store; pick another if
     taken)
   - **Primary language**
...
xcodebuild -project Hushbox.xcodeproj -scheme Hushbox \
  -configuration Release \
  -archivePath build/Hushbox.xcarchive \
...
# then upload build/export/Hushbox.ipa with Transporter.app or:
xcrun altool --upload-app -f build/export/Hushbox.ipa -t ios \
...
App Store Connect ▸ **App Privacy**. Hushbox is on-device with no accounts,
...
```bash
# Simulator
xcodegen generate && open Hushbox.xcodeproj      # then ⌘R

# Tests (same as CI)
xcodebuild test -project Hushbox.xcodeproj -scheme Hushbox \
  -destination 'platform=iOS Simulator,name=iPhone 16'

# Archive for upload
xcodebuild -project Hushbox.xcodeproj -scheme Hushbox -configuration Release \
  -archivePath build/Hushbox.xcarchive -destination 'generic/platform=iOS' archive
```
