# App Store Connect submission checklist (Quietbox)
...
> need a choice from you; the recommended answer for Quietbox is given.
...
- ℹ️ **Minimum iOS: 26.** Quietbox uses Liquid Glass, so only devices on iOS 26+
...
- ☐ **Platform:** iOS
- ☐ **Name:** `Quietbox` (must be unique across the App Store — have a backup
...
- ☐ **App Privacy** ("nutrition label") — click **Get Started**, then
  **decision → select "No, we do not collect data from this app."** Quietbox
...
- ☐ **US BIS — the one external legal item.** Uploading to Apple is an *export
  ...
  Quietbox is a free, mass-market product using standard crypto, which fits
